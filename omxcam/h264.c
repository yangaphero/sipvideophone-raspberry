...
int omxcam__h264_configure_omx (omxcam_h264_settings_t* settings){
  ...
  if ((error = OMX_SetParameter (omxcam__ctx.video_encode.handle,
	...
	omxcam__omx_struct_init (tProfileLevel);
	tProfileLevel.nPortIndex = 201;
	if ((error = OMX_GetParameter (omxcam__ctx.video_encode.handle, OMX_IndexParamVideoProfileLevelCurrent,&tProfileLevel)))
	{
		omxcam__error ("--- OMX_GetParameter for video encode output port failed with:%s\n", omxcam__dump_OMX_ERRORTYPE (error));
		return -1;
	}
	tProfileLevel.eProfile = settings->profile;
	tProfileLevel.eLevel = OMX_VIDEO_AVCLevel42;
	if ((error = OMX_SetParameter (omxcam__ctx.video_encode.handle, OMX_IndexParamVideoProfileLevelCurrent,	&tProfileLevel)))
	{
		omxcam__error ("--- OMX_SetParameter for video encode output port failed with: %s\n",omxcam__dump_OMX_ERRORTYPE (error));
		return -1;
	}
